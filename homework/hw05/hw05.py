...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        return tree(label(t), [replace_leaf(num, old, new) for num in branches(t)])
...
def balanced(m):
    ...
    if is_weight(m):
        return True
    side1, side2 = sides(m)[0], sides(m)[1]
    return length(side1) * total_weight(end(side1)) == length(side2) * total_weight(end(side2)) and balanced(end(side1)) and balanced(end(side2))
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        count, copy_bal = 0, self.balance
        while copy_bal < amount:
            copy_bal = copy_bal * (1 + self.interest)
            count += 1
        return count
