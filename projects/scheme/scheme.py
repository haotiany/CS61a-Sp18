...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            macros = procedure.apply_macro(rest, env)
            return scheme_eval(macros, env)
        vals = rest.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(procedure, vals, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('number of arguments must match with number of parameters')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        Frame.define(env, target.first, LambdaProcedure(target.second, expressions.second, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        return target.first
    bad_target = target.first if isinstance(target, Pair) else target
    raise SchemeError('{0} is not a symbol'.format(bad_target))
    # END Problem 21
